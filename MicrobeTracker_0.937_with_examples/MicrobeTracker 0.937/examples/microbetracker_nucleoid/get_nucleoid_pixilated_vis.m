...
for cell=1:length(cellList{1})
    if ~isempty(cellList{1}{cell})
        % obtain the cell/nucleoid masks (area is not computed)
        ...
        imshow(cellmask*0.5+nucleoidmask*0.5); % display the image
        hold on % draw the cell outline on top of the image
        plot(x0,y0,'y') % draw cell outline
        hold off
        set(gca,'pos',[0 0 1 1]) % maximize image size within the figure
        1;
    end
end
