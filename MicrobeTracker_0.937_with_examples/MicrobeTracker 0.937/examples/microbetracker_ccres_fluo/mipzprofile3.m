    ...
    for cell=1:length(cellList{frame}) % cycle through all cells on this frame
        if ~isempty(cellList{frame}{cell}) % only consider the cells that exist
            if ~isempty(cellList{frame}{cell}.divisions) && cellList{frame}{cell}.divisions(end)==frame ... % only consider the cells that just divided
                ...
                count = count+1;
                disp(['Event ' num2str(count) ': frame=' num2str(frame) ', cell=' num2str(cell)])
            end
        end
    end
end
meanprofile = mean(profilearray(1:5,:)); % calculate mean profile
mx = find(meanprofile>meanprofile([2 1:end-1])&meanprofile>meanprofile([2:end end-1])); % find local maxima
[mnv,mn] = min(meanprofile(mx(1):mx(end))); mn = mn+mx(1)-1 % find the global minimum between the first and the last local maxima
xvector = 0:1/50:1;
figure;
plot(xvector,meanprofile,'-',xvector(mx),meanprofile(mx),'.',xvector(mn),meanprofile(mn),'.')